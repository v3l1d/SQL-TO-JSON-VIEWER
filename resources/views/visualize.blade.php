  ...
  success: function(tables){
      ...
      operators.forEach(function(item) {
          ...
          item.inputs.forEach(function(input) {
              let inp = "input_" + j
              data['operators'][operator]['properties']['inputs'][inp] = {label: input}
              let obj = {}
              obj[input] = inp
          ...
          item.outputs.forEach(function(output) {
              ...
              data['operators'][operator]['properties']['outputs'][outp] = {label: col}
              let obj = {}
              obj[col] = outp;
              ...
              links_table[item.name].outputs.forEach(function(elem) {
                  if (elem[output.column]) {
                      ...
                      if (links_table[elem["table"]]) {
                          ...
                          links_table[elem["table"]].inputs.forEach(function(inp) {
                              if (inp[elem["field"]]) {
      ...
      $('#example_2').flowchart({
          ...
          onOperatorSelect: function(operatorId) {
              openNav($flowchart, operatorId, $flowchart.flowchart('getOperatorData', operatorId));
              return true;
          }
      });
  },
